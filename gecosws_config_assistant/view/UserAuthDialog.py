...
class UserAuthDialog(GladeWindow):
    ...
    def setFormForLDAP(self):
        ...
        self.biglabel.set_text(
            _("Please fill the data to setup the LDAP authentication method."))
    ...
    def setFormForAD(self):
        ...
        self.biglabel.set_text(
            _("Please fill the data to setup the Active Directory " +
    ...
    def setFormForSpecificAD(self):
        ...
        self.biglabel.set_text(
            _("Specific configuration data was loaded from GECOS server\n" +
    ...
    def setFormForInternal(self):
        ''' Setting form for Internal '''

        self.logger.debug("Setting form for Internal")

        self.biglabel.set_text(
            _("There is no necessary data to setup the local users " +
              "authentication method."))
    ...
    def _show_active_directory_method(self):
        ...
        # Fill data values
        if (
            self.get_data() is not None and
            isinstance(self.get_data(), ADAuthMethod)
        ):
    ...
    def _show_ldap_method(self):
        ...
        if (
            self.get_data() is not None and
            isinstance(self.get_data(), LDAPAuthMethod)
        ):
            data = self.get_data().get_data()
            if (
                data.get_uri() is not None and
                data.get_uri().strip()!=''
            ):
                self.entry1.set_text(data.get_uri())

            if (
                data.get_base() is not None and
                data.get_base().strip()!=''
            ):
    ...
    def setup(self, *args):
        ...
        if self.controller.test():
            if not self.controller.save():
                showerror_gtk(
                    _("An error happened while saving the users " +
                      "authentication method!") + "\n" +
    ...
    def _populate_data(self):
        ...
        if index == LDAP_USERS:
            ...
            if (
                self.get_data() is not None and
                isinstance(self.get_data(), ADAuthMethod) and
                self.get_data().get_data().get_specific()
            ):
                # Specific AD setup
            ...
            setupData.set_ad_administrator_pass(
