...
from gecosws_config_assistant.view.CommonDialog import (
    askyesno_gtk, showerror_gtk, showinfo_gtk)
from gecosws_config_assistant.view.ADSetupDataElemView import (
    ADSetupDataElemView)
from gecosws_config_assistant.view.UserAuthDialog import (
    UserAuthDialog, LOCAL_USERS, LDAP_USERS, AD_USERS)

from gecosws_config_assistant.util.Validation import Validation
from gecosws_config_assistant.dao.UserAuthenticationMethodDAO import (
    UserAuthenticationMethodDAO)
...
class UserAuthenticationMethodController(object):
    ...
    def get_auth_data_from_conf(self, conf):
        ...
        if (
            conf["auth"]["auth_type"] != 'AD' and
            conf["auth"]["auth_type"] != 'LDAP'
        ):
            self.logger.error(
    ...
    def _setup_ldap_authentication_method(self, conf):
        ...
        if conf["auth"]["auth_properties"]["uri"] == 'URL_LDAP':
            self.logger.info(
                'Default LDAP URI detected. Skip user authentication setup!')
            return True
    ...
    def _data_has_changed(self):
        ...
        if type(oldData).__name__ != type(newData).__name__:
            self.logger.debug('Old data: %s New data:%s',
                              type(oldData).__name__, type(newData).__name__)
            return True
        ...
        if isinstance(oldData, LDAPAuthMethod):
            ...
            if (
                oldAuthData.get_bind_user_dn() != \
                newAuthData.get_bind_user_dn()
            ):
                self.logger.debug('LDAP bind user DNs are different')
                return True

            if (
                oldAuthData.get_bind_user_pwd() != \
                newAuthData.get_bind_user_pwd()
            ):
    ...
    def accept(self):
        ...
        if self._data_has_changed():
            if (
                askyesno_gtk(
                    _('The user authentication data has changed.\n' +
                      'Do you want to setup the user authentication ' +
                      'method using this new data?'), self.view)
            ):
                # Test and save
                if self.test():
                    result = self.save()
    ...
    def save(self):
        ...
        if newData is not None:
            ...
            if isinstance(oldData, ADAuthMethod):
                ...
                # Ask the user for Active Directory administrator
                # user and password
                askForActiveDirectoryCredentialsView = ADSetupDataElemView(
                    self.mainWindow, self)
                askForActiveDirectoryCredentialsView.set_data(
                    oldData.get_data())
        ...
        if isinstance(newData, LDAPAuthMethod):
            ...
            if (newAuthData.get_uri() is None or
                ...
                showerror_gtk(
                    ...
                    _("Please fill all the mandatory fields."), self.view)
                self.view.focusLdapUriField()
                return False

            if not Validation().isLdapUri(newAuthData.get_uri()):
                self.logger.debug("Malformed LDAP URI! %s",
                                  newAuthData.get_uri())
                showerror_gtk(
                    _("Malformed LDAP URI!") + "\n" +
                    _("Please check that the URI starts with 'ldap(s)://'."),
                     self.view)
                self.view.focusLdapUriField()
                return False
            ...
            if not newAuthData.get_specific():
                ...
                if ipaddress is None:
                    showerror_gtk(
                        ...
                        "\n" +
                        _("Please check the Domain field " +
